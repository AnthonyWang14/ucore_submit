...
alloc_proc(void) {
    ...
    proc->lab6_priority = 1;
     //LAB5 2013011356 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2013011356 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2013011356 HINT:need add some code to init fs in proc_struct, ...
    proc->filesp = NULL;
    }
    return proc;
}
...
find_proc(int pid) {
    if (0 < pid && pid < MAX_PID) {
        // cprintf("hash 2 res is: %d ---\n", pid_hashfn(pid));
        list_entry_t *list = hash_list + pid_hashfn(pid), *le = list;
        while ((le = list_next(le)) != list) {
            struct proc_struct *proc = le2proc(le, hash_link);
            // cprintf ("one proc id is 0x%08x\n", proc->pid);
            if (proc->pid == pid) {
                return proc;
            }
        }
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2013011356  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    assert(current != NULL && current->wait_state == 0);
    assert ((proc = alloc_proc()) != NULL);
    proc->parent = current;
