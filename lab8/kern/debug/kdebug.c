...
print_stackframe(void) {
      ...
      */
    uint32_t r_ebp = read_ebp();
    uint32_t r_eip = read_eip();
    int i;
    for (i = 0; r_ebp != 0 && i < STACKFRAME_DEPTH; i++) {
        cprintf("ebp:0x%08x eip:0x%08x args:0x%08x 0x%08x 0x%08x 0x%08x\n", \
            r_ebp, r_eip, ((uint32_t*)r_ebp)[2], ((uint32_t*)r_ebp)[3], \
            ((uint32_t*)r_ebp)[4], ((uint32_t*)r_ebp)[5]);
        print_debuginfo(r_eip - 1);
        r_eip = ((uint32_t*)r_ebp)[1];
        r_ebp = ((uint32_t*)r_ebp)[0];
    }
}
